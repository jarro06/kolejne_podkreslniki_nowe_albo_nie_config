...
def job_config = [
        job: [
                name: "kolejne_podkreslniki_nowe_albo_nie_Pipeline_develop",
                agent: "maven"
        ],
        git: [
                branch: "develop"
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                   jslGit.checkout( config, "jarro06", "kolejne_podkreslniki_nowe_albo_nie", job_config.git.branch)
               }
            }    
        }
        stage('Build') {
...
def job = pipelineJob("kolejne_podkreslniki_nowe_albo_nie_Pipeline_develop");
