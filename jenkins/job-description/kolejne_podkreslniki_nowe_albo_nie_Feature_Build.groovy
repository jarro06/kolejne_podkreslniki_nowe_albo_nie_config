...
def job_config = [
    job: [
        name: "kolejne_podkreslniki_nowe_albo_nie_Feature_Build",
        agent: "maven"
    ]
]
def config = [
...
ghe: [
        ...
        GitGroup: 'jarro06',
        RepoName: 'kolejne_podkreslniki_nowe_albo_nie',
        checks: [
...
def customLib = library identifier: 'kolejne_podkreslniki_nowe_albo_nie_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/jarro06/kolejne_podkreslniki_nowe_albo_nie_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.KolejnePodkreslnikiNoweAlboNie.new()
    ...
    stage('Prepare') {
      steps {
        ...
        script {
          ...
          jslGit.checkout( config, "jarro06", "kolejne_podkreslniki_nowe_albo_nie", branch_name)
          }
        }
      }
stage('Build') {
...
"""
def job = pipelineJob("kolejne_podkreslniki_nowe_albo_nie_Feature_Build");
...
job.with {
    ...
    authenticationToken('0tS3wDatflOMFfUMneLVdbDdagiyGQI8')
